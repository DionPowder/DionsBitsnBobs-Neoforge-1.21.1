...
public class RubyChocolateGlazingType implements FanProcessingType {
    ...
    public boolean isValidAt(Level level, BlockPos pos) {
        ...
        if (fluidState.getType().builtInRegistryHolder().is(DionsBitsnBobsTags.Fluids.FAN_PROCESSING_CATALYSTS_RUBY_CHOCOLATE_GLAZING)) return true;
        var blockState = level.getBlockState(pos);
        return blockState.getBlock().builtInRegistryHolder().is(DionsBitsnBobsTags.Blocks.FAN_PROCESSING_CATALYSTS_RUBY_CHOCOLATE_GLAZING);
    }
    ...
    public boolean canProcess(ItemStack stack, Level level) {
        ...
        return recipeManager
                .getRecipeFor(DionsBitsnBobsRecipeTypes.RUBY_CHOCOLATE_GLAZING.getType(), input, level)
                .isPresent();
    }
    ...
    public void morphAirFlow(AirFlowParticleAccess particleAccess, RandomSource random) {
        particleAccess.setColor(Color.mixColors(0xE667D5, 0xCC66BE, random.nextFloat()));
