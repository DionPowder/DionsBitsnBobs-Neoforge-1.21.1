...
public class OrangeFrostingType implements FanProcessingType {
    ...
    public boolean isValidAt(Level level, BlockPos pos) {
        ...
        if (fluidState.getType().builtInRegistryHolder().is(DionsBitsnBobsTags.Fluids.FAN_PROCESSING_CATALYSTS_ORANGE_FROSTING)) return true;
        var blockState = level.getBlockState(pos);
        return blockState.getBlock().builtInRegistryHolder().is(DionsBitsnBobsTags.Blocks.FAN_PROCESSING_CATALYSTS_ORANGE_FROSTING);
    }
    ...
    public boolean canProcess(ItemStack stack, Level level) {
        if (!DionsBitsnBobsConfig.recipes().BULK_FROSTING.get())
            return false;
        var recipeManager = level.getRecipeManager();
        var input = new SingleRecipeInput(stack);
        return recipeManager
                .getRecipeFor(DionsBitsnBobsRecipeTypes.ORANGE_FROSTING.getType(), input, level)
                .isPresent();
    }
    ...
    public void morphAirFlow(AirFlowParticleAccess particleAccess, RandomSource random) {
        particleAccess.setColor(Color.mixColors(0xEB8E2A, 0xF09D48, random.nextFloat()));
